...
export const Route = createFileRoute("/popular/movies")({
...
function PopularMoviesPage() {
  ...
  useEffect(() => {
    async function load() {
      setDataLoading(true);
      const { data } = await supabase.from("media_items").select("*").eq("media_type", "movie");
      if (data?.length) {
        ...
        setItems(withAvg.filter((m) => m.review_count > 0).sort((a, b) => b.review_count - a.review_count).slice(0, 25));
      }
    ...
    <AppShell>
      <section className="px-5 pt-5">
        ...
        <div className="mt-4 flex items-center gap-2">
          ...
          <h1 className="text-2xl font-extrabold tracking-tight">Popular Movies</h1>
        </div>
        <p className="mt-1 text-sm text-muted-foreground">Top 25 most reviewed movies</p>
      </section>
      ...
      <section className="mt-5 px-5 pb-8">
        {dataLoading ? (
          ...
          <p className="py-10 text-center text-sm text-muted-foreground">No movies rated yet.</p>
        ) : (
